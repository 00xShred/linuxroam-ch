#!/usr/bin/env bash
# linuxroam-ch — Swiss eduroam installer for Linux + NetworkManager.
# Based on linuxroam by Diverso Lab: https://github.com/diverso-lab/linuxroam
# Pulls the institution profile from eduroam CAT, but deliberately leaves
# NetworkManager's CA certificate field unset for Swiss profiles that document
# "no certificate needed" (e.g. ETHZ/ZHAW GUI instructions).
#
...
# Usage:
#   ./install.sh
#   ./install.sh --country CH
#   ./install.sh --profile N
...
Options:
  --country XX   CAT federation code (defaults to CH).
...
BANNER
  printf '%s' "$RST" >&2
  printf '%s  Swiss eduroam on Linux — no GUI certificate toggle%s\n\n' "$DIM" "$RST" >&2
  printf '  %sBased on:%s    https://github.com/diverso-lab/linuxroam\n' "$BLD" "$RST" >&2
  printf '  %sData source:%s eduroam CAT (cat.eduroam.org)\n' "$BLD" "$RST" >&2
  printf '  %sSwiss mode:%s  leaves CA certificate unset by default\n\n' "$BLD" "$RST" >&2
}
...
show_details() {
  ...
  printf '   %sCA cert:%s     none (Swiss mode)\n' "$BLD" "$RST" >&2
...
disclaimer() {
  ...
  printf '   %s•%s This installs a Swiss %seduroam%s Wi-Fi profile and stores your\n' "$CYN" "$RST" "$BLD" "$RST" >&2
...
spin "$EAP_XML" "Downloading institution profile" \
...
# Swiss mode intentionally leaves CA certificate and domain validation unset.
# ponytail: if another country needs strict validation later, add --strict-ca.
NM_DOMAIN=""
...
exec sudo -E \
    ...
    OUTER_ID="$OUTER_ID" DISPLAY="$DISPLAY" \
    bash -s <<'ROOT'
set -euo pipefail
...
ok_step "Swiss mode: no CA certificate installed"
